...
//				//insertMatchs
	...
	public List<totalDetailDto> getTotalDetail(String MatchId, String puuid) {
		List<selectDetailDto>list1 = dao4.selectDetail(MatchId);
		matchsDto dto = dao4.selectMatchs(MatchId);
		List<totalDetailDto>list2 = new ArrayList<>();
		String realTime = ""+(dto.getPlayTime()/60)+":"+(dto.getPlayTime()%60);
		int cnt=0;
		System.out.println("MatchId: " + MatchId);
		LocalDateTime endTime = LocalDateTime.parse(dto.getEndTime(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.S"));
		long days = ChronoUnit.DAYS.between(endTime, LocalDateTime.now());
		String result = days + "일 전";
		for(int i=0; i<=list1.size()-1; i++) {
			if(puuid.equals(list1.get(i).getPuuid())) {
				System.out.println("list1 summonerId : "+(i+1)+"번째"+list1.get(i).getMdSummonerId());
				cnt = i;
			}
		}
		String winOrLosses = "패배";
		int summonerTeam = list1.get(cnt).getTeamId();
		int winnerTeam =  dao.getOnlyWinnerTeam(MatchId);
		if(summonerTeam == winnerTeam) {
			winOrLosses = "승리";
		}
		String SummonerId = list1.get(cnt).getMdSummonerId();
		System.out.println("MatchId : " +  MatchId);
		System.out.println("SummonerId : " + SummonerId);
		MatchSpellDto spellDto = dao4.getMatchSpell(MatchId, SummonerId);
		System.out.println(spellDto.getMatchSpell1());
		String spell1 = dao2.selectSpellUrl(spellDto.getMatchSpell1());
		String spell2 = dao2.selectSpellUrl(spellDto.getMatchSpell2());
		System.out.println(spell1);
		
		MatchRunesDto summonerRune = dao4.getMatchRune(MatchId, SummonerId);
		String Rune1 = dao2.selectRuneUrl(summonerRune.getMatchMainRune1());
		System.out.println("Rune1 URl : "+Rune1);
		int subRuneType = dao.getOnlyMatchRuneType(summonerRune.getMatchMainRune4());
		System.out.println(subRuneType);
		String Rune2 = "error";
		switch (subRuneType) {
		case 1: Rune2 = "https://cdn.lol.ps/assets/img/runes2/8000_40.webp"; break;
		case 2: Rune2 = "https://cdn.lol.ps/assets/img/runes2/8100_40.webp"; break;
		case 3: Rune2 = "https://cdn.lol.ps/assets/img/runes2/8200_40.webp"; break;
		case 4: Rune2 = "https://cdn.lol.ps/assets/img/runes2/8400_40.webp"; break;
		case 5: Rune2 = "https://cdn.lol.ps/assets/img/runes2/8300_40.webp"; break;
		}
		
		//여기 작업해야해
		for (int i=0; i<=list1.size()-1; i++) {
			MatchItemsDto dto2 = dao4.selectMatchItem(MatchId, list1.get(i).getMdSummonerId());
			...
			totalDetailDto totaldto = new totalDetailDto(
					...
					URL, result, realTime, dto.getWinnerTeam(), winOrLosses, spell1, spell2, Rune1, Rune2);
			list2.add(totaldto);
			if(puuid.equals(list1.get(i).getPuuid())) {
				cnt = i;
